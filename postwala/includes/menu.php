...
////////////////////////////////////////////////////////////
function generateLatestItems($location='',$limit=5){//displays the latest ads of all location if location chosen display that location ads alone
	if (COUNT_POSTS){
		$ocdb=phpMyDB::GetInstance();
	    ...
	    $query="select P.*,C.friendlyName,(select friendlyName from ".TABLE_PREFIX."categories where idCAtegory=C.idCategoryParent Limit 1) parent
                    from ".TABLE_PREFIX."categories C inner join
			            (SELECT count(P.idPost) cont,P.idPost,P.title,P.type,P.idCategory FROM ".TABLE_PREFIX."posts P						            
				            where P.isConfirmed=1 and P.isAvailable=1 and P.title!='' group by P.idPost,P.title,P.idCategory,P.type) P
                on P.idCategory=C.idCategory
				order by idPost desc Limit $limit";
		$result=$ocdb->getRows($query);
		...
		foreach ($result as $row ) {
			...
			$fcategory=$row["friendlyName"];
			$parent=$row["parent"];
			$postTitle=ucfirst(strtolower($row["title"]));
			$FpostTitle=friendly_url($postTitle);
			
			if (is_numeric($idPost)){
				$url=itemURL($idPost,$fcategory,$postTypeName,$FpostTitle,$parent);
				$pop.="<li style='padding-bottom:15px;'><a title=\"$postTitle $postTypeName $fcategory\" href=\"".SITE_URL."$url\">$postTitle</a></li>";
			}
		}
		$pop.="</ul>";
	}
...
function generatePopularItems($days=7,$limit=5,$idCategory){//displays the top X limit popular items from the last X days, possible by category
	if (COUNT_POSTS){
		...
		foreach ($result as $row ) {
			...
			if (is_numeric($idPost)){
				$url=itemURL($idPost,$fcategory,$postTypeName,$FpostTitle,$parent);
				$pop.="<li style='padding-bottom:15px;'><a title=\"$hitsCount $postTitle $postTypeName $fcategory\" href=\"".SITE_URL."$url\">$postTitle</a></li>";
			}
		}
		$pop.="</ul>";
	}
