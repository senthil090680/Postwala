...
function accountCheck($key,$id=""){ //try to prevent duplicated accounts
    ...
    if (is_numeric($id)) $query="SELECT EMAIL FROM classifieds_visitor where (EMAIL='$key') and (USERID <> $id) limit 1";
    else $query="SELECT email FROM classifieds_visitor where (EMAIL='$key') limit 1";
...
<table>
	...
	<?php
		if ($_POST){
            ...
            if (is_numeric($account_id)){
                if ($action=="edit"){
                    ...
                    $ocdb->update(classifieds_visitor,"name='$name',email='$email'","USERID=$account_id");
            ...
            if (is_numeric($account_id)){
                if ($action=="activate"){
                    $ocdb->update(classifieds_visitor,"ISVALID='Y'","USERID=$account_id");

                    $action_notify=T_("Activated");

                    echo "<div id='sysmessage'>".T_("Account successfully activated")."</div>";
                } elseif ($action=="deactivate"){
                    $ocdb->update(classifieds_visitor,"ISVALID='N'","USERID=$account_id");
                    ...
                    $ocdb->delete(classifieds_visitor,"USERID=$account_id");
        ...
        $order="a.REGISTERATION_DATE Desc";

        //pagination
        $query="SELECT count(a.USERID) AS total FROM classifieds_visitor a
                WHERE 1 $filter";
        ...
        $query="SELECT a.USERID,b.NAME,a.EMAIL,a.REGISTERATION_DATE,a.ISVALID
                FROM classifieds_visitor a, classifieds_visitorinfo b
                WHERE a.USERID = b.USERID $filter
        ...
        if ($resultSearch){
			$row_count = 0;
			foreach ( $resultSearch as $row ){
				$idAccount=$row['USERID'];
				$name=$row['NAME'];//
				$email=$row['EMAIL'];//
				$insertDate=setDate($row['REGISTERATION_DATE']);

				if($row['ISVALID'] == "Y") $active = 1;
				else $active = 0;
